...
<li class="bc-text ub ub-ver word-wrap ut-m line<%=option.multiLine%>"><%=data.title%></li>\
<%if(data.describe){%><li class="ulev-1 sc-text word-wrap uinn3"><%=data.describe%> </li><%}%>\
<%if(data.note){%><li class="ulev-2 sc-text uinn3"><%=data.note%></li><%}%>\
</ul>\
<%if(option.hasSubTitle){%>\
<ul class="ub ub-ver ub-ae umar-r">\
<%if(data.subTitle){%><li class="bc-text word-wrap lv_subTitle"><%=data.subTitle%></li><%}%>\
<%if(data.subDescribe){%><li class="ulev-1 sc-text word-wrap uinn3"><%=data.subDescribe%></li><%}%>\
<%if(data.subNote){%><li class="ulev-2 sc-text uinn3"><%=data.subNote%></li><%}%>\
</ul>\
<%}%>\
<%if(option.hasControl && data.switchBtn){%>\
<div class="switch uba bc-border <%if(data.switchBtn.mini){%>switch-mini<%}%>" data-checked="<%=data.switchBtn.value%>">\
